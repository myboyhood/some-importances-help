...
using namespace cv;
vector<float> param;
int thrd_val,dilate_val,erode_val,cir_dilate_size,cir_erode_size,contour_linewidth,circle_minR,circle_maxR,isshow,isimwrite,write_analyze_result;
float scale_coeffi;
Mat imgsrc;
...
bool data_is_right = false;
vector<float> a_data, b_data,  d_data, e_data, f_data, aerfa_data;
float a_avg, b_avg,  d_avg, e_avg, f_avg, aerfa_avg;
float a_error=0.0,b_error=0.0,d_error=0.0,e_error=0.0,f_error=0.0,aerfa_error=0.0;
float a_stddev,b_stddev,d_stddev,e_stddev,f_stddev,aerfa_stddev;
int g_n;//image good number
vector<int> b_n;//image bad number
ofstream measureresult;
...
void on_measure(int i)
{
    ...
    if(line1 > line2)
    {
        ...
        cout << "ahavg " << ahavg <<"   bhavg " << bhavg << "   mhavg " << mhavg << endl;
        if(horiflag == 1)
        {
    ...
    else{data_is_right = false;}
    if(data_is_right == true)
    {
...
//    f = lo9*sin_theta_1;
    ...
    a_data.push_back(a);b_data.push_back(b);d_data.push_back(d);e_data.push_back(e);f_data.push_back(f);aerfa_data.push_back(aerfa);
    }
    else
    {
        a_data.push_back(0.0);b_data.push_back(0.0);d_data.push_back(0.0);e_data.push_back(0.0);f_data.push_back(0.0);aerfa_data.push_back(0.0);
        b_n.push_back(i);////define the number of bad piece
    }
}
...
int main(int argc, char *argv[])
{
    ...
    cout << "input piecenumber: 1 2 3 4" << endl;
    cin >> piece_number;
    thrd_val = param[0];
    dilate_val = param[1];
    erode_val = param[2];
    ...
    isimwrite = 0;
    isimwrite = param[10];
    write_analyze_result = param[11];
//    String imagename = "/home/wzy/softwares/CLion-2019.3/projects/workpiecemeasure/imagename.txt";
//    ifstream fina(imagename);
//    int image_count = 0;
//    string filename;
//    if(!fina.good())
//    {
//        while (getline(fina, filename))
//        {
//            image_count++;
//            imageshub.emplace_back(imread(filename));
//        }
//    }
    ...
    for (int j = 0; j < imageshub.size(); ++j)
    {
        ...
        if(isimwrite)
        {   string imagefile = "/home/wzy/softwares/CLion-2019.3/projects/workpiecemeasure/image_saved/";
            String name = imagefile+to_string(j)+"  threshold.jpg";
            imwrite(name,imgthrd);
        }
...
////calculate right point
        ...
        circle(drawinglinecolor, Point(circlepoint.x, circlepoint.y),circleRadius, Scalar(20, 10, 200), 3, LINE_AA);
...
//        line(drawinglinecolor, Point(l[0], l[1]), Point(l[2], l[3]), Scalar(186, 88, 255), 1, LINE_AA);
        ...
        if(isimwrite)
        {   string imagefile = "/home/wzy/softwares/CLion-2019.3/projects/workpiecemeasure/image_saved/";
            String name = imagefile+to_string(j)+"  detect_result.jpg";
            imwrite(name,drawinglinecolor);
        }
    ...
    g_n = a_data.size()-b_n.size();
    for (int i = 0; i < g_n; ++i)
    {
        a_avg += a_data[i];b_avg += b_data[i];d_avg += d_data[i];e_avg += e_data[i];f_avg += f_data[i];aerfa_avg += aerfa_data[i];
    }

    a_avg /= g_n;b_avg /= g_n;d_avg /= g_n;d_avg /= g_n;e_avg /= g_n;f_avg /= g_n;aerfa_avg /= g_n;

    for (int i = 0; i < g_n; ++i)
    {
        a_error += pow(a_data[i]-a_avg,2);b_error+=pow(b_data[i]-b_avg,2);d_error+=pow(d_data[i]-d_avg,2);
        e_error+=pow(e_data[i]-e_avg,2);f_error+=pow(f_data[i]-f_avg,2);aerfa_error+=pow(aerfa_data[i]-aerfa_avg,2);
    }
    a_stddev = sqrt(a_error / (g_n-1));
    b_stddev = sqrt(b_error / (g_n-1));
    d_stddev = sqrt(d_error / (g_n-1));
    e_stddev = sqrt(e_error / (g_n-1));
    f_stddev = sqrt(f_error / (g_n-1));
    aerfa_stddev = sqrt(aerfa_error / (g_n-1));

    if(write_analyze_result)
    {
        measureresult.open("/home/wzy/softwares/CLion-2019.3/projects/workpiecemeasure/analyze_result.txt",ios::app);
        for (int i = 0; i < a_data.size(); ++i)
        {
            if(a_data[i] < 1.0)//mean a_data == 0.0
            {
                measureresult << " bad data, skip this sample " << endl;
                continue;
            }
            ...
            {
                measureresult << "  " << endl;
                measureresult << "      # "+ to_string(i+1) << endl;
                measureresult << "ID , data "<< endl;
                measureresult << "a = " << a_data[i]/scale_coeffi << endl;
                measureresult << "b = " << b_data[i]/scale_coeffi << endl;
                measureresult << "aerfa = " << aerfa_data[i] << endl;
                measureresult << "d = " << d_data[i]/scale_coeffi << endl;
                measureresult << "e = " << e_data[i]/scale_coeffi << endl;
                measureresult << "f = " << f_data[i]/scale_coeffi << endl;
            }
    ...
    finalresult.open("/home/wzy/softwares/CLion-2019.3/projects/workpiecemeasure/final_result.txt",ios::app);
    finalresult << "  " << endl;
    finalresult << to_string(piece_number)+"# "  << endl;
    finalresult << "ID , Average , StdDev "<< endl;
    finalresult << "a , " << a_avg/scale_coeffi <<" , " << a_stddev/scale_coeffi << endl;
    finalresult << "b , " << b_avg/scale_coeffi <<" , " << a_stddev/scale_coeffi << endl;
    finalresult << "c , " <<aerfa_avg << " , " << aerfa_stddev <<endl;
    finalresult << "d , " << d_avg/scale_coeffi <<" , " << d_stddev/scale_coeffi << endl;
    finalresult << "e , " << e_avg/scale_coeffi <<" , " << e_stddev/scale_coeffi << endl;
    finalresult << "f , " << f_avg/scale_coeffi <<" , " << f_stddev/scale_coeffi << endl;
    finalresult.close();
    waitKey(0);
    return 0;
}
