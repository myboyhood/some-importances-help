...
using namespace cv;
vector<float> param;
int thrd_val,dilate_val,erode_val,cir_dilate_size,cir_erode_size;
Mat imgsrc;
...
void on_measure(int i)
{
    ...
    result << "a = " << a/81.0 << endl;
    result << "b = " << b/81.0 << endl;
    result << "aerfa = " << aerfa << endl;
    result << "d = " << d/81.0 << endl;
    result << "e = " << e/81.0 << endl;
    result << "f = " << f/81.0 << endl;
    result.close();
...
int main(int argc, char *argv[])
{
    ...
    for (int j = 0; j < imageshub.size(); ++j)
    {
        ...
        for (int i = 0; i < contours.size(); ++i) {
            drawContours(drawing, contours, i, Scalar(0, 0, 0), 8, 18, hierarchy, 0, Point());
        }
